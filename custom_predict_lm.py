...
postfix = lm_loss
model_path = base_path + 'multi_' + version + '_' + postfix + '_' + optimizer + '_' + \
...
start_id = 0
if resume:
    with open(base_path + version + '.csv') as prev:
        ...
        for idx, row in enumerate(lines):
            ...
            if idx % 2 == 0:
                count_out1.append(row)
            else:
                count_out2.append(row)

            print(id, row)

        start_id = len(lines) // 2

thread_list = []
count_pr = np.zeros((4, 3))
for idx, row in data.iterrows():
    if resume and idx < start_id:
        continue
    ...
    if not is_parallel:
        count11, pr11 = compute_precision_recall(y_true=parts_lm, y_pred=prediction1, threshold=threshold1,
                                                 conf_score=conf_score, is_sota=is_sota)
        count12, pr12 = compute_precision_recall(y_true=parts_lm, y_pred=prediction1, threshold=threshold2,
                                                 conf_score=conf_score, is_sota=is_sota)
        count_pr[0] = [count11, pr11[0], pr11[1]]
        count_pr[1] = [count12, pr12[0], pr12[1]]
    else:
        t = Thread(target=batch_compute_precision_recall,
                   args=(parts_lm, prediction1, threshold1, conf_score, is_sota, 0, count_pr))
        t.start()
        thread_list.append(t)

        t = Thread(target=batch_compute_precision_recall,
                   args=(parts_lm, prediction1, threshold2, conf_score, is_sota, 1, count_pr))
        t.start()
        thread_list.append(t)
    ...
    if not is_parallel:
        count21, pr21 = compute_precision_recall(y_true=parts_lm, y_pred=prediction2, threshold=threshold1,
                                                 conf_score=conf_score, is_sota=is_sota)
        count22, pr22 = compute_precision_recall(y_true=parts_lm, y_pred=prediction2, threshold=threshold2,
                                                 conf_score=conf_score, is_sota=is_sota)
        count_pr[2] = [count21, pr21[0], pr21[1]]
        count_pr[3] = [count22, pr22[0], pr22[1]]
    else:
        t = Thread(target=batch_compute_precision_recall,
                   args=(parts_lm, prediction2, threshold1, conf_score, is_sota, 2, count_pr))
        t.start()
        thread_list.append(t)

        t = Thread(target=batch_compute_precision_recall,
                   args=(parts_lm, prediction2, threshold2, conf_score, is_sota, 3, count_pr))
        t.start()
        thread_list.append(t)
...
with open(model_path + '_' + str(threshold1) + '_' + str(conf_score) + postfix, 'w') as csv_file:
    for out1 in count_out1:
        csv_file.write(out1 + '\n')

with open(model_path + '_' + str(threshold2) + '_' + str(conf_score) + postfix, 'w') as csv_file:
