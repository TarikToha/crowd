...
postfix = lm_loss
model_path = 'multi_' + version + '_' + postfix + '_' + optimizer + '_' + str(INPUT_SIZE) + '_' + \
...
for idx, row in data.iterrows():
    ...
    start = time.time()
    parts, _, parts_lm = density_map_patches(file_name=file_name, INPUT_SIZE=INPUT_SIZE, dm_factor=1,
                                             scale=1, np_path=np_path, num_dir=num_dir, bench_name=bench_name)
    com = time.time()
    prediction = predict_multi_dm(img_array=parts, model=model1, is_multi=False, is_sota=is_sota)
    ...
    count11, pr11 = compute_precision_recall(y_true=parts_lm, y_pred=prediction, threshold=threshold1,
                                             conf_score=conf_score, is_sota=is_sota)
    count12, pr12 = compute_precision_recall(y_true=parts_lm, y_pred=prediction, threshold=threshold2,
                                             conf_score=conf_score, is_sota=is_sota)

    prediction = predict_multi_dm(img_array=parts, model=model2, is_multi=False, is_sota=is_sota)
    # count2 = np.sum(prediction[0])
    # count2 = np.sum(prediction[1])
    end2 = time.time()
    # res2 = compute_psnr_ssim(y_true=parts_dm, y_pred=prediction[0])
    count21, pr21 = compute_precision_recall(y_true=parts_lm, y_pred=prediction, threshold=threshold1,
                                             conf_score=conf_score, is_sota=is_sota)
    count22, pr22 = compute_precision_recall(y_true=parts_lm, y_pred=prediction, threshold=threshold2,
                                             conf_score=conf_score, is_sota=is_sota)

    out1 = file_name.split('/')[-1] + ',' + str(count11) + ',' + str(pr11[0]) + ',' + str(pr11[1]) + ',' + \
           str(end1 - start) + ','

    out1 += str(count21) + ',' + str(pr21[0]) + ',' + str(pr21[1]) + ',' + str(end2 - end1 + com - start)

    count_out1.append(out1)
    print(idx, out1)

    out2 = file_name.split('/')[-1] + ',' + str(count12) + ',' + str(pr12[0]) + ',' + str(pr12[1]) + ',' + \
           str(end1 - start) + ','

    out2 += str(count22) + ',' + str(pr22[0]) + ',' + str(pr22[1]) + ',' + str(end2 - end1 + com - start)
