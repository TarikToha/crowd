...
if not resume:
    ...
    model = BatchNormalization()(model)
    model = LeakyReLU()(model)
    model = Conv2D(filters=64, kernel_size=(3, 3), dilation_rate=(2, 2), kernel_initializer=init, padding="same")(model)
    model = BatchNormalization()(model)
    model = LeakyReLU()(model)
    model = Conv2D(filters=32, kernel_size=(3, 3), dilation_rate=(2, 2), kernel_initializer=init, padding="same")(model)
    model = BatchNormalization()(model)
    model = LeakyReLU()(model)
