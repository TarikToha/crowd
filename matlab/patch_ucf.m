...
% folder = '/media/user/Data/odrive/grad/traffic_signal/data/UCF_CC_50/';
folder = '/media/user/Data/odrive/grad/traffic_signal/code/ucf_18/';
% grid = 7;
image_size = 256;
patch_dir_num = 20;

txt = fopen(strcat(folder, 'ucf_18_labels.csv'), 'w');
...
for i=1:total
    ...
    %     name = char(images(i));
    name = 'test_img_0100.jpg';
    
    base_name = split(name, '.');
    mat_file = char(strcat(folder, 'ground_truth/', base_name(1), '_ann.mat'));
    ...
    for y = 1:ystep:h
        for x = 1:xstep:w
            ...
            patch_dir = strcat('patch', int2str(mod(count, patch_dir_num)), '/', cpim_name);
            
            %             imwrite(cp_im, strcat(folder, patch_dir));
            
            grid_count = gt_patch(mat_file, xmin, xmax, ymin, ymax);
            
            fprintf(txt, '%s,%d\n', patch_dir, grid_count);
        end
    end
    disp(name);
    fprintf('%d / %d = %f\n', i, total, 100*i/total);
