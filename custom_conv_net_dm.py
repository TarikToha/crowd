...
# dataset_path = '/media/toha/Data/odrive/grad/traffic_signal/dataset/shanghai/patch_256/'
# dataset_path = '/content/drive/My Drive/traffic_signal/dataset/shanghai/patch_256/'
dataset_path = 'dataset/shanghai/patch_256/'
# dataset_path = 'dataset/ucf_18_hajj/patch_256/'
version = 'v6'
dm_loss = 'msep'
optimizer = 'adam'
INPUT_SIZE = 256
...
train_generator = DataGenerator(dataframe=train, bench_name=bench_name, batch_size=batch_size, im_size=INPUT_SIZE,
                                dm_factor=DM_FACTOR, mode='train', scale=1, is_dm=True, is_lm=False)

valid_generator = DataGenerator(dataframe=valid, bench_name=bench_name, batch_size=batch_size, im_size=INPUT_SIZE,
                                dm_factor=DM_FACTOR, mode='valid', scale=1, is_dm=True, is_lm=False)
...
if not resume:
    ...
    # 256
    model = Conv2D(filters=64, kernel_size=(3, 3), padding="same")(inputs)
    ...
    model = Conv2D(filters=512, kernel_size=(3, 3), dilation_rate=(2, 2), padding="same")(model)
    model = BatchNormalization()(model)
    model = LeakyReLU()(model)
    model = Conv2D(filters=512, kernel_size=(3, 3), dilation_rate=(2, 2), padding="same")(model)
    model = BatchNormalization()(model)
    ...
    density_map_output = Conv2D(filters=1, kernel_size=(1, 1), activation="relu", padding="same")(model)

    ##############################################################

    # construct the CNN
    model = Model(inputs, density_map_output)

    # model = init_from_vgg(model)
...
if optimizer == 'sgd':
    optimizer = SGD(lr=1e-7, momentum=0.95, decay=5e-4)
